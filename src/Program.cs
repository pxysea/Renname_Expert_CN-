...
namespace RenameTrn
{
    ...
    public class Trans
    {
        public List<Item> Translation = new List<Item>();
    }
    class Program
    {
        /// <summary>
        /// Renname Expert 自动翻译文
        /// 复制安装目录下的 languages.xml (原始XML文件)
        /// </summary>
        /// <param name="args"></param>
        static void Main(string[] args)
        {
            //原始文件,5.28.1版本
            string dexml = @"D:\Users\Sivyer\Desktop\Rename Expert 5.28.1 中文破解\languages.xml";
            //引用一下 5.20 版本，翻译过的就不再翻译了,未翻译的使用Google翻译
            string cnxml = @"D:\Users\Sivyer\Desktop\Rename Expert 5.28.1 中文破解\cn.txt";

            //输出路径
            string outpath = @"D:\Users\Sivyer\Desktop\Rename Expert 5.28.1 中文破解\cnout.txt";

            Trans tout = new Trans();

            

            Trans de = XmlHelper.ReadXML<Trans>(new StringReader($"<Trans>{dexml}</Trans>"));
            Trans cn = XmlHelper.ReadXML<Trans>(new StringReader($"<Trans>{cnxml}</Trans>"));

            string fanyi = "";

            foreach(Item item in de.Translation)
            {
                string str = item.lngDe;
                string strcn = item.lngEn;
                bool flag= false;
                foreach (Item itemcn in cn.Translation)
                {
                    if (itemcn.lngDe.Equals(str))
                    {
                        strcn = itemcn.lngEn;
                        flag = true;
                        break;
                ...
                if (!flag)
                {
                    List<string> list = Google(item.lngEn,"zh");
                    if (list.Count > 1)
                    {
                        strcn = list[1];
                        fanyi += $"{item.lngEn}  --> {strcn}\r\n";
                        Thread.Sleep(1000);
                    }
                }
                tout.Translation.Add(new Item() { lngDe = item.lngDe, lngEn = strcn }); 
            }
            File.WriteAllText($"{outpath}.fanyi.txt", fanyi);
            XmlHelper.WriteXML(tout,outpath,"");
        ...
        /// <param name="target">zh</param>
        /// <returns></returns>
        public static List<string> Google(string text,string target)
        {
            ...
            List<string> result = MatchResultAsList(content);
            return result;
        }
        ...
        public static List<string> MatchResultAsList(string content)
        {
            string patttern = @",\[\[\\\""(.*?)\\\"",";
            Regex regex = new Regex(patttern);
            MatchCollection matchcollection = regex.Matches(content);
    ...
    public static class XmlHelper
        ...
        /// <param name="path"></param>
        /// <returns></returns>
        public static T ReadXML<T>(TextReader file)
        {
            try
            {
                var reader = new XmlSerializer(typeof(T));
        ...
        public static void SaveTableToFile(DataTable dataTable, string filePath)
            ...
            /// <returns></returns>
            public static string WriteXML<T>(T item, string path,  string ends)
            {
